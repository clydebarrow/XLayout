...
open class HorizontalLayout(layout: Layout = Layout(), vararg views: View) : ViewGroup(layout) {
    ...
    override fun onMeasure(parentWidth: Double, parentHeight: Double) {
        ...
        val sizeMeasured = CxFactory.cxSize()
        ...
        if (layoutHeight == MAX_DIMENSION) {
            ...
            childViews
                    ...
                    .forEach { v ->
                        v.measure(layoutWidth, MAX_DIMENSION)
                        sizeMeasured.height = Math.max(sizeMeasured.height, v.measuredSize.height + v.layout.margins.totalHeight())
                        logMsg("$name: sizemeasured is %s after adding %s", sizeMeasured.asSizeString(), v.name)
                    }
            layoutHeight = sizeMeasured.height
        } else
            sizeMeasured.height = layoutHeight
        indent -= 4
        logMsg("$name: layoutHeight now " + Layout.dimToString(layoutHeight))
        indent += 4
        ...
        childViews
                ...
                .forEach { v ->
                    when (v.layout.widthMode) {
                        Layout.Mode.MatchParent -> error("width MatchParent inside horizontal makes no sense")
                        Layout.Mode.Weighted -> totalWeight += v.layout.weight
                        else -> {
        ...
        indent -= 4
        if (totalWeight != 0.0 && layout.widthMode == Layout.Mode.WrapContent)
            logMsg("$name: using weights inside WrapContent makes no sense")
        // adjust for spacing
        ...
        if (layout.widthMode == Layout.Mode.WrapContent || layoutWidth == MAX_DIMENSION) {
            // if the width is not constrained, we can't flexibly lay things out.
        ...
        measuredSize = layout.resolveSize(CxFactory.cxSize(layoutWidth, layoutHeight), sizeMeasured)
    ...
    override fun onLayout(newPosition: CxRect, parentHidden: Boolean) {
        ...
        childViews.forEachIndexed { idx, v ->
            // Hide hidden views
            if (v.gone) {
                v.performLayout(CxFactory.cxRect(), false)
            } else {
                ...
                val y: Double
                y = when (g) {
                    Gravity.Vertical.Bottom -> newPosition.maxY - v.layout.margins.bottom - size.height
                    Gravity.Vertical.Center -> (newPosition.minY + newPosition.maxY - size.height) / 2
                    else -> newPosition.minY + v.layout.margins.top
                }
                logMsg("$name: layout ${v.name} size = $size, y = $y")
                v.performLayout(CxFactory.cxRect(x, y, size.width, size.height), false)
